...
void PID_Calculate(pid_t *target, float Error)
{
	...
	target->err = Error;
	target->err_LPF = 0.6f * target->err + 0.4f * target->err_LPF;

	if(target->err_LPF+1==target->err_LPF)
	{// 判断NAN
		target->err_LPF = target->err;
	}
	...
	if(target->I_Dis==0)
	{
		...
		target->out = target->kp * Error + target->i_value + target->kd * (target->err_LPF - target->err_old);
	}
	else
	{
		target->out = target->kp * Error + target->i_value + target->kd * (target->err_LPF - target->err_old);
